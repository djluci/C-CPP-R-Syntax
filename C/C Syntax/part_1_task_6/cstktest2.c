...
char *intToString(void *x) {
    int *intValue = (int *)x; // Casts the void pointer to an int pointer
    char *str = (char *)malloc(12 * sizeof(char)); // Allocating enough memory for a string representation of an int
    sprintf(str, "%d", *intValue); // converts integer to string 
    return str; //returns string
}

char *account_toString(void *v) {
    Account *account = (Account *)v; // casts void pointer to an Account pointer
    char *str = (char *)malloc(100 * sizeof(char)); // Allocating enough memory for a string representation
    sprintf(str, "%s: %d", account->name, account->balance);
    return NULL;
}
